...
predBayes<-predict(modelo, newdata = train[,1:80])
...
Según el modelo planteado, ha habido un error en la predicción de 67 casas a precio de venta económico que ha sido predicho como intermedio, 14 de precio intermedio que se han predicho como caras y 4 de precio intermedio que se han predicho como económicas. Esto quiere decir que el modelo tiene un porcentaje de acierto del 80.64% para predicir la clasificación según los precios de venta de las casas.
...
#na.action(train)
train[is.na(train)] <- 0
ct<-trainControl(method = "cv",train[,1:80],number=10, verboseIter=T)
#modeloCaret<-train(y~.,data=train,method="nb",trControl = ct)
#prediccionCaret<-predict(modeloCaret,newdata = test[,1:81])
